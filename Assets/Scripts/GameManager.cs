...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0) || (Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved))
        {
            ...
            if (wait)
            {
                ...
                // up
                if (gap.y > 0 && gap.x > -0.5f && gap.x < 0.5f) for (x = 0; x <= 3; x++) for (y = 0; y <= 2; y++) for (i = 3; i >= y + 1; i--)
                                MoveOrCombine(x, i - 1, x, i);
                // down
                else if (gap.y < 0 && gap.x > -0.5f && gap.x < 0.5f) for (x = 0; x <= 3; x++) for (y = 3; y >= 1; y--) for (i = 0; i <= y - 1; i++)
                                MoveOrCombine(x, i + 1, x, i);
                // right
                else if (gap.x > 0 && gap.y > -0.5f && gap.y < 0.5f) for (y = 0; y <= 3; y++) for (x = 0; x <= 2; x++) for (i = 3; i >= x + 1; i--)
                                MoveOrCombine(i - 1, y, i, y);
                // left
                else if (gap.x < 0 && gap.y > -0.5f && gap.y < 0.5f) for (y = 0; y <= 3; y++) for (x = 3; x >= 1; x--) for (i = 0; i <= x - 1; i++)
                                MoveOrCombine(i + 1, y, i, y);
                ...
                if (move)
                {
                    ...
                    if (k == 0)
                    {
                        // 가로, 세로 같은 블럭이 없으면 l이 0이 돼서 게임오버
    ...
    // 이동 전 좌표 = [x1, y1]; 이동 할 좌표 = [x2, y2]
    void MoveOrCombine(int x1, int y1, int x2, int y2)
    {
        // 이동 할 좌표가 비어있고, 이동 전 좌표에 존재하면 이동
